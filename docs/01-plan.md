...
| Snowflake · Tasks | `contoso-data-product-snowflake-tasks` | `snowflake-platform-tasks` | 🔴 | gold-only over an empty seeded silver; emulator has no Tasks. **Also fails DoD 1**: its platform pins core by sibling path (`path = "../contoso-data-product"`), so it cannot be cloned and built alone — and it is the one consumer that needed no v0.1.1 bump, because it has no pin. See G11 |
...
| G13 | **One platform carries its own copy of the vendors.** `fabric-platform-notebook-pipelines` tracks all eight vendor definition files under `sources/` and has its own `materialise_sources.py`; it references `contoso-sources` nowhere. `00-family.md` says every platform generates its vendor stack from that repo and none carries a vendor of its own — this cell predates it and was never moved. The eight files are **byte-identical today**, which is exactly why nothing has noticed: they agree by accident of history, not by structure, and the first edit to a spec in `contoso-sources` silently makes this cell pull different bytes from every other cell. That would not fail — it would just quietly stop meaning anything, since identical inputs are the whole basis for comparing runtimes. | the Fabric · notebooks cell's claim to be comparable | platform | delete `sources/` and `scripts/materialise_sources.py`; generate the vendor stack from `contoso-sources/sources.yaml` as the other two platforms already do |
