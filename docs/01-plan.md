...
| G15 | **The two runtimes publish the product under DIFFERENT domains.** `databricks-platform-jobs` imports `DOMAIN` from core (`contoso-commerce`, "Contoso Commerce"); `fabric-platform-notebook-pipelines` hardcodes `DOMAIN = "contoso-sales"` and `GLOSSARY = "Contoso Sales"` at govern.py:236-237 and imports nothing. The HUMAN catalog therefore shows one product in two domains, described two ways — the disagreement OpenMetadata exists to remove, reproduced inside it. | the catalog's claim to describe one product | platform, then a one-line product choice | **The structural half is not a decision.** Core already names the domain and `RULES.md` §1 says a platform consumes what core names rather than restating it, so the fabric platform is in breach whichever string is right — the same breach as G13, fixed by deleting the copy rather than choosing between copies. It imports `DOMAIN` and `GLOSSARY` from core, full stop. That leaves one open question worth a line: keep `contoso-commerce`, or have core adopt `contoso-sales`? Arguments both ways — core names it today; fabric published it first, and "match what is already published" is the rule that settled `domainType` an hour earlier |
