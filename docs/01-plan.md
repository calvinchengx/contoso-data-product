...
| G15 | **The two runtimes publish the product under DIFFERENT domains.** `databricks-platform-jobs` imports `DOMAIN` from core (`contoso-commerce`, "Contoso Commerce"); `fabric-platform-notebook-pipelines` hardcodes its own `DOMAIN = "contoso-sales"` ("Contoso Sales") and imports nothing. So the HUMAN catalog shows one product living in two domains, described two ways — which is the disagreement OpenMetadata is there to remove, reproduced inside it. Same shape as G13 one tier over: a platform holding its own copy of something core already names. | the catalog's claim to describe one product | platform + core | fabric's govern imports `DOMAIN` from `contoso_product.contracts`; whichever name wins is a **product** decision, not a platform one, and renaming touches both platforms and core |
